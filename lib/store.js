...
import { initialBanners, initialFeatured, initialCategories, initialCoupons, initialOrders, initialSettings, initialExpenses, initialReminders } from './initialData';
...
function initializeMemoryStore() {
  ...
  memoryStore.set('reminders', JSON.parse(JSON.stringify(initialReminders)));
...
export async function getReminders() { return (await getData('reminders')) || []; }
export async function setReminders(reminders) { return setData('reminders', reminders); }
...
// ==================== WAITER REQUESTS ====================
export async function getWaiterRequests() {
  return (await getData('waiterRequests')) || [];
}

export async function setWaiterRequests(requests) {
  await setData('waiterRequests', requests);
}
